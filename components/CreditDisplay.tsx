...
const TwitterIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" {...props}><path d="M9.294 6.928L14.357 1h-1.2L8.762 6.116L5.33 1H1.242l5.363 7.64L1.242 15h1.2l4.6-5.4L11.67 15h4.089l-5.736-8.072h-.729Z"/></svg>
);
const ArtstationIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M2 12.356L8.351 2h1.6L16 16.273l-2.121 4.242L2 12.356zM15.449 2L22 12.569l-4.242 8.485L15.449 2z"/></svg>
);
const FacebookIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M14 13.5h2.5l1-4H14v-2c0-1.03 0-2 2-2h1.5V2.14c-.326-.043-1.557-.14-2.857-.14C11.928 2 10 3.657 10 6.7v2.8H7v4h3V22h4v-8.5Z"/></svg>
);
const TumblrIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M14.563 21c-3.59 0-6.149-2.2-6.149-5.543V9.923H6V8.012h2.414V5.127c1.314-1.2 3.2-1.85 5.086-1.85h2.829v3.007h-2.143c-.43 0-.686.2-.686.643v2.09h2.829l-.43 1.91h-2.4v5.337c0 1.987 1.286 3.097 3.2 3.097h1.5v2.828h-2.414Z"/></svg>
);
...
export const CreditDisplay: React.FC<CreditDisplayProps> = ({ credit, className }) => {
  ...
  const hasContent = Object.values(credit).some(value => !!value);
  if (!hasContent) {
    return null;
  }
  
  const socialLinks = [
    { url: credit.websiteUrl, Icon: WebsiteIcon, label: 'Website' },
    { url: credit.twitterUrl, Icon: TwitterIcon, label: 'Twitter/X' },
    { url: credit.artstationUrl, Icon: ArtstationIcon, label: 'ArtStation' },
    { url: credit.facebookUrl, Icon: FacebookIcon, label: 'Facebook' },
    { url: credit.tumblrUrl, Icon: TumblrIcon, label: 'Tumblr' },
    { url: credit.instagramUrl, Icon: InstagramIcon, label: 'Instagram' },
    { url: credit.pinterestUrl, Icon: PinterestIcon, label: 'Pinterest' },
    { url: credit.deviantartUrl, Icon: DeviantArtIcon, label: 'DeviantArt' },
    { url: credit.behanceUrl, Icon: BehanceIcon, label: 'Behance' },
    { url: credit.linkedinUrl, Icon: LinkedInIcon, label: 'LinkedIn' },
    { url: credit.vimeoUrl, Icon: VimeoIcon, label: 'Vimeo' },
    { url: credit.youtubeUrl, Icon: YouTubeIcon, label: 'YouTube' },
    { url: credit.pixivUrl, Icon: PixivIcon, label: 'Pixiv' },
    { url: credit.dribbbleUrl, Icon: DribbbleIcon, label: 'Dribbble' },
    { url: credit.itchioUrl, Icon: ItchioIcon, label: 'Itch.io' },
    { url: credit.otherUrl, Icon: LinkIcon, label: 'Other Link' },
        ...
        <div className="space-y-6">
          {credit.title && (
            <div>
              ...
              <p className="text-[var(--text-primary)] text-lg">{credit.title}</p>
            </div>
          )}
          {credit.artistName && (
            <div>
          ...
          {socialLinks.length > 0 && (
            <div>
              <p className="font-semibold text-[var(--text-secondary)]">Links</p>
              <div className="flex flex-wrap items-center gap-x-6 gap-y-3 mt-2">
                {socialLinks.map(({ url, Icon, label }) => (
                    <a key={label} href={url!} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-accent)] transition-colors group">
