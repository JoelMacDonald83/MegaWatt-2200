...
export const StyleSelect: React.FC<{label: string, value: string | undefined, onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void, children: React.ReactNode, wrapperClassName?: string, disabled?: boolean, help?: string}> = ({label, value, onChange, children, wrapperClassName, disabled, help}) => (
    <div className={wrapperClassName}>
        ...
        <select value={value} onChange={onChange} disabled={disabled} className="w-full bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded-md p-2 text-[length:var(--font-size-sm)] disabled:bg-[var(--bg-panel-light)] disabled:text-[var(--text-tertiary)] disabled:cursor-not-allowed">
            {children}
        </select>
    </div>
);
